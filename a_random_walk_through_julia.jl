...
# ╔═╡ ad3c7738-3a75-42fc-a40d-aebc74e3ddc1
using Enzyme
...
Bottum-up view point. (I may go too much into the weeds).

With material from the Julia community particularly Mosè Giordano (RSE@UCL).
"""
...
1. Use `juliaup` or binaries from https://julialang.org/downloads
2. Julia is column major and starts from 1
3. https://docs.julialang.org/en/v1/manual/getting-started/
4. https://docs.julialang.org/en/v1/manual/noteworthy-differences/
5. https://docs.julialang.org/en/v1/manual/performance-tips/
6. https://docs.julialang.org/en/v1/stdlib/Pkg/
7. discourse.julialang.org
...
# ╔═╡ c849bcc9-5682-4080-9e65-332f0cafcaaa
md"""
...
# ╔═╡ 67a9781b-86c7-4e1f-8dc7-934760260b9a
mandel(complex(x_real, x_img))

# ╔═╡ 223e763a-2878-497b-9934-dc75f4dff0b2
md"""
...
# ╔═╡ 78b96615-66c5-48e6-ae41-044c877da3a4
...
# ╔═╡ 8a8c0ee9-e0d8-40ec-b01b-731df0a9a78f
...
1. System image
2. Application deployment through PackageCompiler.jl
...
- https://docs.julialang.org/en/v1/devdocs/pkgimg/
...
Monthly call on the 4th Tuesday of the Month! See https://julialang.org/community/

Julia supports common HPC architectures:

- x86_64
- AArch64
- PowerPC

as well as many HPC accelerators:

- NVIDIA GPU
- AMD GPU
- Intel GPU

Julia is being used for large-scale HPC applications:
...
# ╔═╡ ed65bbf0-4d3d-47a9-8b38-01e5284a1940
md"""
...
# ╔═╡ 80ae0a39-831f-4182-831e-f53a5cc6057f
Enzyme.Compiler.enzyme_code_llvm(stdout, sin, Active, Tuple{Active{Float64}})

# ╔═╡ fd1c7308-6a05-4070-aa2a-f6c24ebd75df
md"""
### Rosenbrock function 🗻
...
# ╔═╡ 67c12718-e2d5-45cc-9fd5-b0861c8dd2c1
md"""
...
# ╔═╡ 46b76fc7-705c-401d-99c4-be8e92c54ed6
md"""
### Minimising a function with Enzyme 📉

We can run a program to find the minima of the Rosenbrock function on a large grid of different starting points, and plot the number of iterations taken before stopping, when the terminating criteria are met.
In particular, we will use the [Adam](https://arxiv.org/abs/1412.6980), an optimisation method which requires the gradient of the function to optimise as input, and we will use Enzyme to synthesize it on the host system at compile time.
"""

# ╔═╡ c30b8552-03f4-4f79-b4c0-7179ead69192
rosenbrock(x, y) = (1 - x) ^ 2 + 100 * (y - x ^ 2) ^ 2;

# ╔═╡ 99f196eb-a902-40a0-a446-de797290e3ab
begin
	xs = -6:0.04:6
	ys = -2:0.04:5
...
# ╔═╡ 1cad4984-c7b0-4066-b4a0-523e44926b57
surface(xs, ys, zs, axis=(type=Axis3,))

# ╔═╡ 8d38ad28-cb0b-4474-a73e-dc255de3624c
contourf(xs, ys, zs)

# ╔═╡ 3f8f8f6c-d49d-4627-b984-3f1109b133d2
Enzyme.Compiler.enzyme_code_llvm(stdout, 
	rosenbrock, Active, Tuple{Const{Float64}, Active{Float64}},
	debuginfo=:none)

# ╔═╡ 2e05f98d-4be5-4ff3-a4cd-af030664ce5c
∂(f, x, y) = first(autodiff(Enzyme.Reverse, f, Active(x), Active(y)));

# ╔═╡ 03a4d03a-b65d-44bc-baa5-8d4e94555457
rosenbrock′(x, y) = ∂(rosenbrock, x, y)

# ╔═╡ 318a2627-c889-433d-aa4c-07f43bd152ff
rosenbrock′(1.0, 2.0)

# ╔═╡ dea83c81-a922-4c3b-a206-0d1070dd86cf
function optimize!(∂f, x₀::T, y₀::T) where {T}
	x = x₀
	y = y₀
...
# ╔═╡ 981be30e-5b2a-4079-aecf-83be0bfc8211
optimize!(rosenbrock′, 1.0, 2.0)

# ╔═╡ 419f09ed-51c4-496f-bc70-6a7f703a9cb4
begin
...
# ╔═╡ 3db20f87-5a4e-474f-9fc4-aa83e37ca4d5
m = [optimize!(rosenbrock′, x, y) for x in x_range, y in y_range]

# ╔═╡ 778cc943-8202-4ae1-ac24-9ec546dee236
begin
...
git-tree-sha1 = "5a9f3fe3cdec4bfa91120a83eb5366d595385c05"
...
git-tree-sha1 = "d35761b44e1057cbbc94dd8c685b07678eb14053"
